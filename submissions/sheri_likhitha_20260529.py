...
class buffer:
    def __init__(self):
        ...
        self.cond = Condition()   # Condition object for synchronization
...
def  transpose(a):
    ...
    while len(a[0]) > 0:
        ...
        for x in a:
            row.append(x.pop(0))
        b.append(row)   #How  to  stores  results  in  list

    return b        #How  to  return  the  list
		
# End  of  the  function
a = eval(input('Enter  nested  list :  '))
t = transpose(a)        #How  to  call  transpose()  function
print('Transpose matrix :',t)         #How  to  print  transpose
'''
...
def  denom(n):
	b=[]#How  create  a  list
	for  x  in  a:
		b.append(n // x)   # How  to  store  9  results  in  list  with  for  loop
		n = n % x

	return  b   # How  to  return  list
# denom(2878)
a = [500 , 200 , 100 , 50 , 20 , 10 , 5 , 2 , 1]
n = int(input('Enter  any   number:  '))
b = denom(n)   # How  to  call  denom()  function
...
def   arabic_roman(n):
	...
	for  i  in  range(len(a)):
		s = s + (b[i] * (n // a[i]))   # Concatenate  each  result  to  the  string
		n = n % a[i]

	return  s   # return  the  string
n = int(input('Enter  arabic  number  :   '))	
r = arabic_roman(n)   # How  to  call  arbic_roman()  function

print('Roman  number  :  ' , r)   # How  to   print  roman  number