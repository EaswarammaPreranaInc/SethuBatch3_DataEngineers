...
13) What  are  the  outputs  for  t13 . start() ?  ---> f1 method of class 10 times.
'''
...
t2 . start()
'''
outputs are different for different run(can't be predicted)
Rama guess (random number) in attempt: 1
Rama finish in {ctr} attempt
Sita guess (random number) in attempt: 1
