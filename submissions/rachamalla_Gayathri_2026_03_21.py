...
while   True:
	try:
		print(next(f)) #ramarao,Sita,Kiran,Amar,Manohar,Vamshi
		time . sleep(1)
	except:
		break
 # Find  outputs (Home  work)
...
while   True:
	try:
		print(next(f))#{'Roll Num' :  10 , 'Stud Name' : 'Rama Rao' , 'Marks' : 75} , {'Roll Num'  :  15 , 'Stud Name' : 'Kiran' , 'Marks' : 65} , {'Roll Num' :  5 , 'Stud Name' : 'Rajesh' , 'Marks' : 82}
		time . sleep(1)
	except:
		break
# Find  outputs (Home  work)
...
[4/21, 1:15 PM] SRINIVAS Sir Maths: #  Write  a  program  to  print  odd  numbers  between  1  and  20  with  filter  iterator
[4/21, 1:16 PM] SRINIVAS Sir Maths: '''
Write  a  program  to  print  distinct  vowels  of  the  string  using  filter.
Input  is  string  and  output  is  set
'''
[4/21, 1:16 PM] SRINIVAS Sir Maths: # Nested  filter  i.e.  filter  on  filter
import   time
