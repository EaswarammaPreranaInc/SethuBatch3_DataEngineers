...
def disp():  
    # 1. Make child wait until main thread finishes
    main_thread().join()   # Child blocks here till main thread expires
    
    # 2. After main expires, child runs its work
    for i in range(10):
        print('new  thread')
...
# main thread ends here → child gets unblocked and prints "new thread"
...
t1 = Thread(target = double)  # t1 should execute double()
t2 = Thread(target = square)  # t2 should execute square()
