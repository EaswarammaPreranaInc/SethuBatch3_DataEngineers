...
try:
	...
	print('Sec')
	print('Cyb')
#There is no except statements to handle the error and try-except are dependent of each other
...
print('Bye')
'''
error is reported-line 10
Division  by  zero  is  not  permitted
error is reported-line 15
Bye
'''
...
# Identify  error  (Home  work)
        ...
        print('Sec')
        print('Cyb')
#there is no try block to execute the statements present inside the except statements
...
try:
        ...
        print('Two')
        print('Three')
print('Four')#error-statements between try and except are not permitted
except:
		print('Five')
		print('Six')
		print('Seven')
...
try:
	...
	print('Name  Error')
'''error- the default except should be at last of except suite'''
...
'''
error-there should be only one default statement in the program'''
...
print(7 % 0)
'''
all the lines raise ZeroDivisionError because in all the lines division is performed with 0 or 0.0 '''
...
print(a['G']) 
print(a['Y']) #error-there is no key with 'Y' in the dictionary
'''
...
try:
	...
	print('Hyd')
	print(8 / 0)  #error - the error is directly reported and remaining statements are skipped
except:
	print('Sec')
print('Bye')
'''
...
while  True:
	...
	l.delete(i)#How  to  delete  ith  node
	if   i < 0  or  i > a . length():
...
class  singly_linked_list(linked_list):
	def  concat(a , b):
		if  a.start is None:
			a.start=b.start#Result  is  2nd   linked  list 
		else: 
			last=a.start
			while last.link is not None:
				last=last.link
			last.link=b.start #How  to  modify  last  node  link  to  1st  node  of  2nd  linked  list
#  End  of  the  class
...
print("Resultant linked list after concatenation:")
...
for  i   in   range(10):
        print('main  thread')
'''
default target is empty , run method of thread class is executed
output is only main thread'''
...
for  i  in  range(10):
        print('Main  Thread')
child . start()#error-child thread can be executed only once